...
tdl = [["Name:","Age:","Birth Year:","Gender:","Race:","Birthplace:","Home in 1870:","Household Members:"]]
...
def process_data(page, record):
    ...
    with open(path.join(path.join("pages","round" + str(page) + "page" + str(record) + ".html"))) as html: #Open the html file for processing
        ...
        working_list[HOME_BOX] = findput("Home in 1870:", soup)
...
def main():
    ...
    br.follow_link(br.find_link(url_regex="7163"))
    br.open("http://search.ancestrylibrary.com/cgi-bin/sse.dll?db=1870usfedcen&gss=sfs28_ms_db&new=1&rank=1&msT=1&MS_AdvCB=1&gsfn_x=1&gsln_x=1&msbpn__ftp_x=1&msrpn__ftp=New%20York%20City%20(All%20Boroughs)%2C%20New%20York%2C%20USA&msrpn=1652382&msrpn_PInfo=6-%7C0%7C1652393%7C0%7C2%7C3244%7C35%7C1652382%7C0%7C0%7C0%7C&msrpn_x=1&msrpn__ftp_x=1&msypn__ftp_x=1&_83004002_x=1&MSAV=-1&uidh=fph")
    ...
    while path.exists(path.join("pages","round" + str(current_page) + "page" + str(current_record) + ".html")): #Once all the pages are downloaded,
        ...
        if current_record != 19:
            ...
            current_record = 0
    sheet = pe.Sheet(tdl)
    sheet.save_as("1870censusrecords.csv")
