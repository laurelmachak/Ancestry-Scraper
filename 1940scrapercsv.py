...
RECORDS_PER_PAGE  = 20
tdl = [["Name:","Age:","Estimated Birth Year:", "Gender:","Race:","Birthplace:","Marital Status:","Home in 1940:","Street:","Inferred Residence in 1935:","Occupation:","Attended School or College:","Highest Grade Completed:","Income:","Other Income:","Household Members:"]]
...
def process_data(page, record):
    ...
    with open(path.join(path.join("pages","round" + str(page) + "page" + str(record) + ".html"))) as html: #Open the html file for processing
        ...
        working_list[HOME_BOX] = findput("Home in 1940:", soup)
        working_list[STREET_BOX] = findput("Street:",soup)
        working_list[INFERRED_BOX] = findput("Inferred",soup)
        working_list[OCCUPATION_BOX] = findput("Occupation",soup)
        working_list[SCHOOL_BOX] = findput("Attended",soup)
        working_list[GRADE_BOX] = findput("Highest Grade Completed:",soup)
        working_list[INCOME_BOX] = findput("Income:",soup)
        working_list[OTHERINC_BOX] = findput("Income Other Sources:",soup)
...
def main():
    ...
    br.follow_link(br.find_link(url_regex="2442"))
    br.open("http://search.ancestrylibrary.com/cgi-bin/sse.dll?db=1940usfedcen&gss=sfs28_ms_db&new=1&rank=1&msT=1&msrpn__ftp=New%20York%20City%20(All%20Boroughs)%2C%20New%20York%2C%20USA&msrpn=1652382&msrpn_PInfo=6-%7C0%7C1652393%7C0%7C2%7C3244%7C35%7C1652382%7C0%7C0%7C0%7C&msrpn_x=1&msrpn__ftp_x=1&MSAV=0&uidh=fph")
    ...
    while path.exists(path.join("pages","round" + str(current_page) + "page" + str(current_record) + ".html")): #Once all the pages are downloaded,
        ...
        if current_record != 19:
            ...
            current_record = 0
    sheet = pe.Sheet(tdl)
    sheet.save_as("1940censusrecords.csv")
